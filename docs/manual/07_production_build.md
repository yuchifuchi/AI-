...
4. **データソース**：**Amazon S3** を選ぶ。→ 次へ。
   ...
   - **S3の場所**：［参照］でステップ③のバケットを選び、**プレフィックスに `tacit/` を付ける**
     （最終的に `s3://＜バケット名＞/tacit/`）。
   - 解析（パーサ）：**既定のまま**（テキスト抽出）でまずOK。
     ※**PDF内の図・写真・スクショ・スキャン文字まで回答に反映**したいなら、**「高度な解析（Foundation model parsing）」を有効化**（別料金・ページ課金）。
     　ここで有効化してもよいし、**後から `08_image_parsing.md` の手順で追加**もできます（画像入りWord/Excelは**PDFに書き出して**投入するのが前提）。
